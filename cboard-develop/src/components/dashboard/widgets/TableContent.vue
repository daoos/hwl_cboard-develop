      ...
      <div class="box-body" ref="table-body" :style="boxHeight" style="padding: 5px 20px 20px 20px;overflow-y: auto">
        ...
        <div class="mTable-wrapper">
          <table cellspacing="0" cellpadding="0" border="0" class="mTable">
            <thead>
              <tr v-for="(tableTr,index) in headTableData" :key="index">
                <th v-for="(tableTh,index2) in tableTr"
                  :key="index2" :colspan="tableTh.colspan"
                  :class="tableTh.property === 'head-empty' ? 'head-empty' : ''">
                  <div class="cell">{{ tableTh.data }}</div>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(tableTr, index) in realTableData" :key="index">
                <template v-for="(tableTd, childIndex) in tableTr">
                  <td v-if="tableTd.property === 'column-key'" :key="childIndex"
